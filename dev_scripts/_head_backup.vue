  ...
  <div class="atletas-container">
    ...
    <div class="page-header">
      <div class="header-content">
        <div>
          <h1>
            <i class="el-icon-user" /> Gesti├│n de Atletas
          ...
          <p class="subtitle">
            Club Atl├®tico Deportivo Acarigua
              ...
              <div class="filter-popover">
                ...
                <div class="filter-item">
                  ...
                  <el-select v-model="filterCedula" placeholder="Todos" size="small" style="width: 100%">
                    ...
                    <el-option label="Con C├®dula" value="con_cedula" />
                    <el-option label="Sin C├®dula" value="sin_cedula" />
                  </el-select>
                </div>
            ...
            >
              ...
              <div class="athlete-info-compact">
                ...
                <div class="athlete-meta-compact">
                  <span class="athlete-cat-compact" style="margin-right: 10px;">{{ atleta.categoria_nombre || 'Sin categor├¡a' }}</span>
          ...
          <div class="athlete-details-header">
            ...
            <div class="athlete-details-info">
              ...
              <div style="display: flex; gap: 15px; margin-top: 5px; align-items: center;">
                <p style="margin: 0;">Categor├¡a: {{ currentAtleta.categoria_nombre || 'No asignada' }}</p>
                <p style="margin: 0;">Edad: {{ calculateAge(currentAtleta.fecha_nacimiento) }} a├▒os</p>
...
export default {
  ...
  data() {
    return {
      ...
      // Listas para direcci├│n din├ímica
      estadosList: [],
      ...
      tutorRules: {
        ...
        cedula: [
          { required: true, message: 'La c├®dula es requerida', trigger: 'blur' },
          { min: 7, message: 'La c├®dula debe tener al menos 7 caracteres', trigger: 'blur' }
        ],
        telefono: [
        ...
        'direccion.descripcion_descriptiva': [{ required: true, message: 'La direcci├│n detallada es requerida', trigger: 'blur' }]
      }
    }
  },
  computed: {
    ...
    // Obtener pesta├▒as visibles seg├║n el rol
    visibleTabs() {
      return getVisibleAtletasTabs()
    },

    // Verificar si una pesta├▒a espec├¡fica es visible
    isTabVisible() {
      return (tabName) => {
    ...
    datePickerOptions() {
      return {
        disabledDate(time) {
          // No permitir fechas futuras ni menores a 3 a├▒os
          const today = new Date()
  ...
  watch: {
    ...
    filterCedula(newVal) {
      // Resetear el campo de b├║squeda por c├®dula cuando cambia el tipo de filtro
  ...
  methods: {
    ...
    async loadMedicos() {
      try {
        ...
        console.error('Error cargando m├®dicos para el listado:', error)
      }
    },
    ...
    async loadAtletas() {
      this.loadingAtletas = true // Asumiendo que agregaremos un peque├▒o spinner si es necesario
      try {
        const params = {}
        if (this.searchQuery) params.search = this.searchQuery

        // L├│gica de filtro por c├®dula
        ...
        // Si filterCedula === 'todos', no se a├▒aden filtros de c├®dula
    ...
    async selectAtleta(id, keepTab = false) {
      ...
      if (!keepTab) {
        // Si es m├®dico, ir a pesta├▒a m├®dica; si no, a personal
    ...
    openAtletaModal(editing) {
      ...
      if (editing && this.currentAtleta) {
        ...
        // Cargar combos dependientes si es edici├│n
        ...
        if (this.atletaStep === 0) {
          ...
          if (!this.isUnderage) {
            if (!telefono || telefono.length !== 11) {
              this.$message.error('Debe ingresar un n├║mero de tel├®fono v├ílido de 11 d├¡gitos para atletas mayores de edad.')
              return
            }
          } else if (telefono && telefono.length !== 11) {
            this.$message.error('El n├║mero de tel├®fono ingresado est├í incompleto (deben ser 11 d├¡gitos).')
            return
          }

          if (!direccion.estado || !direccion.municipio || !direccion.parroquia || !direccion.descripcion_descriptiva) {
            this.$message.error('Todos los atletas (mayores o menores de edad) deben registrar su direcci├│n completa.')
            return
          }
          ...
          if (this.atletaForm.cedula) {
            if (this.atletaForm.cedula.length < 7 && this.atletaForm.cedula.toUpperCase() !== 'S/N') {
              this.$message.error('La c├®dula del atleta debe tener al menos 7 d├¡gitos (o "S/N").')
              return
            }
            try {
              ...
              if (isDuplicate) {
                this.$message.error('La c├®dula ingresada ya est├í registrada para otro atleta.')
                return
              }
            } catch (error) {
    ...
    checkUnderage() {
      // M├®todo llamado al cambiar la fecha, isUnderage (computed) reaccionar├í a esto autom├íticamente.
      // Se puede a├▒adir l├│gica extra si se requiere limpiar campos del representante, etc.
    },
    ...
    saveAtleta() {
      this.$refs.atletaForm.validate(async(valid) => {
        ...
        // Validaci├│n condicional del representante
        if (this.isUnderage) {
          ...
          if (rep.cedula.length < 7) {
            this.$message.error('La c├®dula del representante debe tener al menos 7 d├¡gitos.')
            return
          }
          if (rep.telefono.length !== 11) {
            this.$message.error('El tel├®fono del representante debe tener exactamente 11 d├¡gitos.')
            return
          }
        } else {
          // Si es mayor de edad, tel├®fono es obligatorio
        ...
        if (!dir.estado || !dir.municipio || !dir.parroquia || !dir.descripcion_descriptiva) {
          this.$message.error('Por favor, complete todos los datos de la direcci├│n de habitaci├│n.')
          return
        }
      ...
      this.$refs.editPersonalForm.validate(async(valid) => {
        ...
        if (!this.isUnderage) {
          if (!this.atletaForm.telefono) {
            this.$message.error('El tel├®fono es obligatorio para atletas mayores de edad.')
            return
          }
          const dir = this.atletaForm.direccion
          if (!dir.estado || !dir.municipio || !dir.parroquia || !dir.descripcion_descriptiva) {
            this.$message.error('Por favor, complete todos los datos de la direcci├│n obligatorios.')
            return
          }
        }
    ...
    async saveMedical() {
      ...
      try {
        ...
        if (this.fichaMedica) {
          ...
          this.$message.success('Ficha m├®dica actualizada')
        } else {
          await request({
            url: '/ficha-medica',
            method: 'post',
          ...
          this.$message.success('Ficha m├®dica creada')
        }
        ...
        try {
          if (this.isEditingTutor && this.tutor) {
            ...
            if (!nuevoTutorId) {
              ...
              throw new Error('No se recibi├│ el ID del tutor creado')
            }
    ...
    async deleteTutor() {
      try {
        await this.$confirm(
          '┬┐Est├í seguro de que desea eliminar este representante? El representante ser├í removido y el atleta quedar├í sin un tercero asignado.',
    ...
    async saveAtencion() {
      if (!this.atencionForm.tipo_registro || !this.atencionForm.especialista_id || !this.atencionForm.descripcion || !this.atencionForm.fecha_suceso) {
        this.$message.warning('Por favor complete todos los campos obligatorios: Tipo, Especialista, Descripci├│n y Fecha.')
        return
      }
      this.loading = true
      try {
        ...
        if (this.isEditingAtencion) {
          ...
          this.$message.success('Atenci├│n m├®dica actualizada')
        } else {
          await request({ url: '/atencion-medica', method: 'post', data })
          this.$message.success('Atenci├│n m├®dica registrada')
        }
    ...
    deleteAtencion(id) {
      this.$confirm('┬┐Desea eliminar este registro m├®dico?', 'Confirmar', {
    ...
    async deletePerformanceTest(testId) {
      if (!testId) return
      try {
        await this.$confirm('┬┐Est├í seguro de eliminar este test de rendimiento?', 'Confirmar', {
    ...
    async deleteMedida(medidaId) {
      if (!medidaId) return
      try {
        await this.$confirm('┬┐Est├í seguro de eliminar estas medidas?', 'Confirmar', {
          confirmButtonText: 'Eliminar',
          cancelButtonText: 'Cancelar',
          type: 'warning'
        })
        ...
        if (error !== 'cancel') {
          ...
          this.$message.error('Error al eliminar medidas antropom├®tricas')
        }
      } finally {
        this.loading = false
      }
    ...
    deleteAtleta() {
      this.$confirm(
        '┬┐Est├ís seguro de que deseas ELIMINAR PERMANENTEMENTE a este atleta? ' +
        'Se borrar├ín TODOS sus registros asociados: asistencias, atenciones m├®dicas, ' +
        'ficha m├®dica, medidas antropom├®tricas, pruebas de rendimiento y su representante. ' +
        'Esta acci├│n NO se puede deshacer. Si solo deseas desactivarlo, cambia su estatus a INACTIVO desde "Editar Datos Personales".',
...
/* Estilos para el input de c├®dula - igual que los selectores */
